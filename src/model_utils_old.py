        ...
        try:
            ...
            try:
                if hasattr(self.model, 'predict_proba'):
                    ...
                    confidence = float(self.model.score(feature_df, None)) if None else None
            except:
                confidence = None
...
def prepare_property_features(raw_input: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if 'Year Remod/Add' in features and 'Year Built' in features:
        features['Years_Since_Remodel'] = features['Year Remod/Add'] - features['Year Built']
    ...
    if 'Overall Qual' in features and 'Overall Cond' in features:
        features['Quality_Condition_Score'] = features['Overall Qual'] * features['Overall Cond']
    
    if 'Garage Area' in features and 'Garage Cars' in features:
        if features['Garage Cars'] > 0:
            features['Garage_Efficiency'] = features['Garage Area'] / (features['Garage Cars'] * 400)
...
def get_property_options_from_dataset(data_path: str) -> Dict[str, list]:
    ...
    try:
        ...
        options = {
            ...
            "building_types": sorted(df['Bldg Type'].unique().tolist()) if 'Bldg Type' in df.columns else [],
            "house_styles": sorted(df['House Style'].unique().tolist()) if 'House Style' in df.columns else [],
