...
class PropertyAdvisor:
    ...
    def analyze(self, property_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not features or predicted_price <= 0:
            raise ValueError("Invalid property_data: requires 'features' and 'predicted_price'")
        
        # Initialize agent state with reasoning tracking
        state = AgentState(
            step=AnalysisState.INIT,
            features=features,
            predicted_price=predicted_price
        )
        state.log_step(f"Analysis started for property in {features.get('neighborhood', 'Unknown')}")
        ...
        state.log_step(f"Property characteristics analyzed ({len(state.analysis.split(chr(10)))} insights)")
        ...
        state.log_step(f"Market position assessed: {state.market.get('market_outlook', 'Unknown')}")
        ...
        report = self._build_comprehensive_report(
            state, recommendation, rag_insights
        )
    ...
    def _generate_recommendation_with_confidence(self, 
    ...
    def _build_comprehensive_report(self, 
                                    state: AgentState,
                                    recommendation: str,
                                    rag_insights: Optional[List[Dict]] = None) -> Dict:
